...
| 1 | Wing stays clear of the menu bar extras | Six sessions fit a 51 pt wing (three columns of two dots); one or two sessions 27 pt | 2026-09-24 | unit (`WingLayoutTests`) + snapshot; whether it clears Connor's own menu bar items is manual |
...
| 2 | Session names | A row shows Claude Code's generated title, a `/rename` replaces it, clearing the rename falls back, no transcript leaves the terminal label; a rename alone rewrites the file | 2026-09-24 | unit (`HookInstallerTests` runs the script against a fixture transcript, jq and plutil; `SessionStoreTests`) + snapshot |
| 2 | Seven sessions | Six dots in three columns and "+1" in the wing; all seven in the open list | 2026-09-24 | unit (`WingLayoutTests`, `SessionStoreTests`); snapshot for six dots, "+N" by unit only |
